...
public:
    ...
    void optimize(){
        ...
        MatrixXd parameter_so_t(6, num_cameras_);
        parameter_so_t = MatrixXd::Zero(6, num_cameras_);
        ...
        for (int j = 0; j < (int)terms.size();j++){
            ...
            C.block(point_id * 3, point_id * 3, 3, 3) += J_ETJ_E;
        }
        ...
        for (int i = 0; i < num_iterations_; i++){
            ...
            for (int j = 0; j < (int)terms.size(); j++){
                ...
                parameter_so_t.col(camera_id) = terms[j]->so_t; //储存so_t
                //cout << parameter_se.col(camera_id) << endl << endl;
                Matrix<double, 2, 9> J_EF = terms[j]->cal_FE(P); //计算E，F
                Matrix<double, 2, 6> J_F = J_EF.block(0, 0, 2, 6);
                Matrix<double, 2, 3> J_E = J_EF.block(0, 6, 2, 3);
            ...
            MatrixXd delta_parameter_cameras_new(6 * num_cameras_, 1); //相机外参的增量
            for (int j = 0; j < num_cameras_;j++){
                delta_parameter_cameras_new.block(6 * j, 0, 6, 1) = parameter_cameras_new.col(j) - parameter_so_t.col(j);
            }
            //cout << delta_parameter_cameras_new << endl;
            ...
            if(method_ == "LM"){
                ...
                double mu = (error_sum - error_sum_next) / h;
                cout << "mu = " << mu << endl;
                if (mu > 0){
                    for (int j = 0; j < num_cameras_;j++){
                        for (int k = 0; k < 6;k++){
                    ...
                    lambda_ *= max((double)(1 / 3), (double)(1 - (2 * mu - 1) * (2 * mu - 1) * (2 * mu - 1)));
                    beta_ = 2;
